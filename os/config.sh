...
setconfig() {
    ...
    if ! test -d "${HOME}/.anyenv/envs/pyenv"; then
        ...
        dvexec "export PATH=\"$HOME/.anyenv/envs/pyenv/shims/:$PATH\""
        dvexec "pyenv install --list"
        dvexec "pyenv install 2.7.10"
        dvexec "pyenv global 2.7.10"
        dvexec "sudo apt-get install -y make build-essential libssl-dev zlib1g-dev libbz2-dev libreadline-dev libsqlite3-dev wget curl llvm"
