...
-- switchbuf をグローバルに設定すると usetab が quickfix 以外 (:stag, <C-w>f, <C-w>F,
-- :sbuffer 系) にも効き、分割せず別タブへ飛ぶ。さらに lsp.lua の <C-]> は
-- tabnew + cfirst で定義へ飛ぶため、定義先が既に開いていると cfirst が元タブへ戻り
-- tabnew した空バッファのタブが残る。quickfix の <CR> だけに閉じるため設定しない。
-- vim.opt.switchbuf = "usetab,newtab" -- quickfix から開く際、既存タブがあれば再利用、無ければ新タブ
-- quickfix から <CR> で選択したときだけ新タブで開く (既存タブがあれば再利用)。
-- location list も filetype=qf のため、:cc (quickfix) と :ll (loclist) を窓の種別で振り分ける。
aumg({
  events = "FileType",
  group = "qf-open-in-tab",
  pat = "qf",
  cb = function(ev)
    vim.keymap.set("n", "<CR>", function()
      ...
      vim.cmd(vim.fn.line(".") .. (is_loclist and "ll" or "cc"))
      vim.o.switchbuf = sb
...
-- wezterm/tmux は曖昧幅を半角(1セル)で扱うため single に統一
-- (Osaka-Mono 側は make-halfwidth-ambiguous.py で曖昧幅記号を半角グリフ化済み)
